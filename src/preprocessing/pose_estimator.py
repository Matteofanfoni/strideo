...
def extract_pose_landmarks_streaming(
    ...
    model_path: Path = MODEL_HEAVY,
    frame_progress: Optional[Callable[[float], None]] = None,
    ...
    Args:
        ...
        start_frame: Source-video frame index to start processing from
            (inclusive). Used by motion gating to skip idle leading
        ...
        model_path: Model bundle passed to ``create_pose_estimator`` when
            ``pose_estimator`` is None (speed-option measurement).
            Ignored if a pre-built ``pose_estimator`` is supplied.
        frame_progress: Optional callback(fraction_0_to_1) fired once per
            extracted frame, so a UI can move a progress bar continuously
            through this pass instead of parking it at the pass's start
            fraction for its whole duration. Separate from
            ``progress_every``, which only prints.
    ...
    # Downsamples a FAST source only: the max(1, ...) clamp means a source
    # slower than target_fps passes through at its own rate rather than being
    # corrected here. That is right in itself -- resampling cannot invent
    # frames the camera never recorded -- but it is only SAFE because
    # ensure_cfr now normalises a constant-but-slow clip to target_fps before
    # anything reaches this line. It previously did not, and a
    # 30 fps upload arrived here and sailed through with skip_rate == 1.
    ...
    while True:
        ...
        if frame_idx % skip_rate == 0:
            ...
            if frame_progress is not None and expected_extracted:
                frame_progress(min(1.0, t / expected_extracted))
...
def extract_pose_landmarks_streaming_reverse(
    ...
    Args:
        ...
        model_path: Model bundle passed to ``create_pose_estimator`` when
            ``pose_estimator`` is None (speed-option measurement).
            Ignored if a pre-built ``pose_estimator`` is supplied.
        frame_progress: Optional callback(fraction_0_to_1) covering *both*
            of this pass's loops, since the RAM-buffering loop is a real
            share of its wall-clock: buffering reports 0 -> BUFFER_WEIGHT,
            detection BUFFER_WEIGHT -> 1.
    ...
    # Downsamples a fast source only; see the note on the same expression in
    # extract_pose_landmarks above. Safe only because ensure_cfr normalises a
    # constant-but-slow clip to target_fps upstream.
    skip_rate = max(1, int(source_fps / target_fps))
    effective_fps = source_fps / skip_rate

    # Share of this pass's progress fraction given to the decode/buffer loop
    # below; the rest belongs to the MediaPipe detection loop. Decoding 4K is
    # cheap next to inference, hence the small slice.
    buffer_weight = 0.25
    expected_buffered = (
        (total_frames + skip_rate - 1) // skip_rate if total_frames > 0 else 0
    )
    ...
    while True:
        ...
        if frame_idx % skip_rate == 0:
            ...
            if frame_progress is not None and expected_buffered:
                frame_progress(
                    buffer_weight * min(1.0, len(frames_rgb) / expected_buffered)
                )
        frame_idx += 1
    cap.release()
    ...
    for t in range(T):
        ...
        if frame_progress is not None:
            frame_progress(buffer_weight + (1.0 - buffer_weight) * ((t + 1) / T))
...
def extract_pose_landmarks_seeded(
    ...
    Args:
        ...
        model_path: Model bundle passed to both sub-pass estimators (

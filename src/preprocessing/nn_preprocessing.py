...
# ---------------------------------------------------------------------------
# Landmark-ordering ablation. Same 17 MediaPipe landmarks as
# MP_TO_NN_INDEX above in every case -- only which NN *slot* each one lands in
# changes -- so this tests whether a 1D-conv kernel's receptive field (which
# joints sit adjacent in the sequence) is doing real work, not whether the
# landmark *set* matters.
# ---------------------------------------------------------------------------

#: MediaPipe index occupying each NN slot 0..16, current production default.
#: Equivalent to [k for k, v in sorted(MP_TO_NN_INDEX.items(), key=lambda kv: kv[1])].
LANDMARK_ORDER_U_SHAPE: Tuple[int, ...] = (
    0,
    15,
    13,
    11,
...
#: Same 17 landmarks, ordered by raw MediaPipe index instead of body contour --
#: the "no ordering design" control arm.
...
#: Registry of the orderings above, keyed by the name threaded through
#: ``StrideoDataset``/``preprocess_for_nn``/``train.py --landmark-order``.
LANDMARK_ORDERINGS: Dict[str, Tuple[int, ...]] = {
    "u_shape": LANDMARK_ORDER_U_SHAPE,
    "mediapipe_raw": LANDMARK_ORDER_MEDIAPIPE_RAW,
    "fixed_random": LANDMARK_ORDER_FIXED_RANDOM,
}


def _mp_to_nn_index_for_order(order: str) -> Dict[int, int]:
    """``{mp_idx: nn_slot}`` for a named ordering -- same shape as MP_TO_NN_INDEX.

    Raises:
        ValueError: If ``order`` is not a key of LANDMARK_ORDERINGS.
    """
    if order not in LANDMARK_ORDERINGS:
        raise ValueError(
            f"Unknown landmark_order {order!r}. "
            f"Expected one of: {sorted(LANDMARK_ORDERINGS)}"
        )
    return {mp_idx: nn_idx for nn_idx, mp_idx in enumerate(LANDMARK_ORDERINGS[order])}


def landmark_role_indices(order: str) -> Tuple[int, int, int]:
    """``(l_hip_slot, r_hip_slot, l_shoulder_slot)`` for a named ordering.

    ``hip_centered_normalization`` needs to know which of the 17 NN slots
    holds the (canonicalised) left hip / right hip / left shoulder under
    whatever ordering is in effect -- those are fixed anatomical roles
    (MediaPipe indices 23/24/11), but their *slot* moves under a
    non-default ordering.
    """
    ordering = LANDMARK_ORDERINGS[order]
    return ordering.index(23), ordering.index(24), ordering.index(11)
...
def extract_nn_landmarks_raw(
    canon_landmarks: np.ndarray,
    canon_visibilities: np.ndarray,
    order: str = "u_shape",
) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    Args:
        ...
        order: Which of LANDMARK_ORDERINGS to place the 17 landmarks in
            . Default ``"u_shape"`` reproduces every prior call
            bit-identically -- same landmark set as MP_TO_NN_INDEX, just
            addressed via the registry instead of the module-level constant.

    Returns:
        nn_landmarks: (T, 17, 2) landmarks in the requested order (pixels)
    ...
    for mp_idx, nn_idx in _mp_to_nn_index_for_order(order).items():
...
# pre_validation's older protocol (v1.5/1.6) used a 5-level pace scheme
# (recovery/steady/threshold/800m) that predates the 3-anchor canonical
# scheme adopted 2026-07-08. recovery/steady have no VDOT-predicted target
# and no anchor of their own; map both onto the slowest anchor so
# normalize_pace() doesn't raise on this data. main_collection's protocol
# only ever emits threshold/1500m/800m, so this alias is a no-op there.
_PACE_LEVEL_ALIASES: Dict[str, str] = {
    "recovery": "threshold",
    "steady": "threshold",
}

# Reference velocities (m/s) for each pace-level anchor, taken as the
...
def resolve_canonical_pace_level(pace_level: str) -> str:
    """Map a raw pace_level string onto one of the 3 canonical anchors.

    Applies the recovery/steady alias; returns other values unchanged
    (including unrecognised ones, so callers still get a clear error from
    whatever they look the result up in).
    """
    return _PACE_LEVEL_ALIASES.get(pace_level, pace_level)
...
def normalize_pace(pace_level: str) -> float:
    ...
    Args:
        pace_level: One of 'threshold', '1500m', '800m' (also accepts the
            older pre_validation 'recovery'/'steady' labels, aliased to
            'threshold' -- see _PACE_LEVEL_ALIASES)
    ...
    """
    resolved = resolve_canonical_pace_level(pace_level)
    if resolved not in PACE_LEVEL_NORMALIZED:
        raise ValueError(
            f"Unknown pace_level '{pace_level}'. "
            f"Expected one of: {list(PACE_LEVEL_NORMALIZED.keys())}"
        )

    return PACE_LEVEL_NORMALIZED[resolved]


def normalize_velocity_continuous(velocity_ms: float) -> float:
    """Continuous conditioning scalar for a raw velocity (m/s).

    Piecewise-linear interpolation through the three
    PACE_LEVEL_NOMINAL_VELOCITY_MS anchors (clamped at the ends by
    ``np.interp``'s own boundary behaviour) -- the same technique
    ``PoleCurve.px_per_m_at_x`` uses for the pole calibration. By
    construction, ``normalize_velocity_continuous(PACE_LEVEL_NOMINAL_VELOCITY_MS[label])
    == normalize_pace(label)`` exactly, so relative-effort conditioning
    collapses to the label baseline whenever the execution factor r == 1.
    """
    labels = ("threshold", "1500m", "800m")
    xp = [PACE_LEVEL_NOMINAL_VELOCITY_MS[k] for k in labels]
    fp = [PACE_LEVEL_NORMALIZED[k] for k in labels]
    return float(np.interp(velocity_ms, xp, fp))
...
def bounded_hybrid_conditioning(pace_level: str, intensity: float) -> float:
    """Bounded-hybrid FiLM conditioning scalar.
    ...
    run time)::
        ...
        scalar = normalize_pace(pace_level)
                 + clip(K * (I - I_REF[pace_level]), -BOUND, +BOUND)
    ...
    adjacent ``normalize_pace`` anchors are 0.22 apart (threshold -> 1500m)
    and 0.23 apart (1500m -> 800m), so a +-0.10 adjustment can never push one
    pace level's scalar into the range of its neighbour.

    Args:
        pace_level: One of 'threshold', '1500m', '800m' (also accepts the
            older pre_validation 'recovery'/'steady' labels, aliased per
            ``resolve_canonical_pace_level`` -- same convention as
            ``normalize_pace``).
        intensity: Capacity-normalised achieved intensity
            ``I = v_measured / v_capacity(runner)`` for this clip.

    Returns:
        The bounded-hybrid conditioning scalar.

    Raises:
        ValueError: If pace_level is not recognised.
    """
    canonical = resolve_canonical_pace_level(pace_level)
    if canonical not in BOUNDED_HYBRID_I_REF:
        raise ValueError(
            f"Unknown pace_level '{pace_level}'. "
            f"Expected one of: {list(BOUNDED_HYBRID_I_REF.keys())}"
        )
    base = normalize_pace(pace_level)
    deviation = BOUNDED_HYBRID_K * (intensity - BOUNDED_HYBRID_I_REF[canonical])
    bounded_deviation = float(
        np.clip(deviation, -BOUNDED_HYBRID_BOUND, BOUNDED_HYBRID_BOUND)
    )
    return base + bounded_deviation


#: Minimum contiguous non-NaN run length worth smoothing. Below this a
#: Gaussian kernel is mostly reaching outside the run and `mode="nearest"`
#: would just replicate the endpoints, so the run is left alone.
_SMOOTH_MIN_RUN_FRAMES = 5


def _contiguous_valid_spans(valid: np.ndarray) -> List[Tuple[int, int]]:
    """``[(start, end), ...]`` half-open spans of contiguous ``True`` in ``valid``.

    Same principle as ``metrics._vo_contiguous_valid_runs``: never
    concatenate samples across a dropout gap, because that silently compresses
    time and a smoothing sigma expressed in frames assumes real time. This
    variant returns **indices** rather than values, since the smoothed result
    has to be written back in place.
    """
...
def _butter_min_span(order: int) -> int:
    """Shortest span ``filtfilt`` can handle at this order.

    ``filtfilt``'s default padding is ``3 * max(len(a), len(b))`` and it requires
    the signal to be strictly longer than that, so a short tracking run has to be
    skipped rather than crash the whole clip.
    """
    return 3 * (order + 1) + 1


def smooth_landmark_series(
    landmarks: np.ndarray,
    sigma: float = 0.0,
    ...
    """Gaussian low-pass each landmark coordinate series over time.

    ``preprocess_for_nn`` historically applied **no** temporal smoothing, so the
    CNN consumed per-frame tracking jitter raw — while the *metrics* path has
    smoothed. BlazePose carries 3-5x RTMPose's high-frequency power,
    concentrated on the distal ankle/heel/toe landmarks that drive contact
    timing, so this is the near-zero-cost test of whether that noise matters
    before any learned denoiser is justified.

    **NaN-aware by construction.** Each of the 34 coordinate series is smoothed
    only *within* its contiguous non-NaN spans, so a dropout gap neither
    propagates NaN across the kernel width nor gets bridged as if the frames
    either side were adjacent.

    **Must be called before ``hip_centered_normalization``**, for two reasons:
    the normalisation reference (hip centre, torso length) is itself derived
    from these landmarks, so smoothing first means the reference inherits it —
    and since hip-centering subtracts the hip position from *all 17* landmarks,
    hip jitter otherwise contaminates every one of them. It must also run
    before ``fill_large_gaps``, or the ``(0, 0)`` fills would bleed into real
    neighbouring frames.

    **Filter shape matters more than filter strength here.** A Gaussian
    has a very gradual rolloff, so buying real attenuation in the noise band
    costs a large fraction of the ~3.3 Hz gait fundamental: at ``sigma=1.5`` it
    removes 93.8% at 15 Hz but also **12.6% at 3.3 Hz**. A 4th-order Butterworth
    at ``cutoff_hz=8`` reaches essentially the same 91.9% attenuation at 15 Hz
    for **0.0%** loss at 3.3 Hz. Cadence was found degrading monotonically
    under Gaussian smoothing while oscillation improved; ``butterworth`` exists
    to test whether that cadence damage was a kernel artifact or whether
    foot-strike transients above the cutoff are genuine signal (the two
    hypotheses predict opposite outcomes).
    ...
    Args:
        ...
        order: Butterworth order. Applied via ``filtfilt``, so the effective
            ...
            important here, because a phase shift would move contact timing.
        fps: Frame rate for the Hz->normalised-frequency conversion. All 45 S1
            clips are exactly 60.0 (verified), which is why this defaults to it.

    Returns:
        ``(smoothed, metadata)``. ``metadata`` records the filter actually
        applied and how many spans were smoothed vs skipped as too short.
    """
    meta: Dict = {
    ...
    if filter_type == "gaussian":
        if sigma <= 0:
            meta["min_run_frames"] = _SMOOTH_MIN_RUN_FRAMES
            return landmarks, meta
        min_len = _SMOOTH_MIN_RUN_FRAMES

        def _apply(seg: np.ndarray) -> np.ndarray:
            return np.asarray(gaussian_filter1d(seg, sigma=sigma, mode="nearest"))

    elif filter_type == "butterworth":
        if cutoff_hz <= 0:
            meta["min_run_frames"] = _butter_min_span(order)
            return landmarks, meta
        nyquist = fps / 2.0
        if not 0.0 < cutoff_hz < nyquist:
            raise ValueError(
                f"cutoff_hz={cutoff_hz} must lie strictly between 0 and the "
                f"Nyquist frequency {nyquist} for fps={fps}"
            )
        b, a = butter(order, cutoff_hz / nyquist, btype="low")
        min_len = _butter_min_span(order)

        def _apply(seg: np.ndarray) -> np.ndarray:
            # filtfilt: zero phase, so contact timing is not shifted.
            return np.asarray(filtfilt(b, a, seg))

    else:
        raise ValueError(
            f"unknown filter_type {filter_type!r} -- expected 'gaussian' or "
            f"'butterworth'"
        )

    meta["min_run_frames"] = min_len
    out = landmarks.copy()
    for lm in range(landmarks.shape[1]):
        for axis in range(landmarks.shape[2]):
            series = landmarks[:, lm, axis]
            for start, end in _contiguous_valid_spans(~np.isnan(series)):
                if end - start < min_len:
                    meta["spans_too_short"] += 1
                    continue
                out[start:end, lm, axis] = _apply(series[start:end])
                meta["spans_smoothed"] += 1
    meta["applied"] = meta["spans_smoothed"] > 0
    return out, meta
...
def preprocess_for_nn(
    ...
    smoothing_cutoff_hz: float = 0.0,
    smoothing_order: int = 4,
    fps: float = 60.0,
    landmark_order: str = "u_shape",
) -> Tuple[np.ndarray, np.ndarray, Dict]:
    """
    ...
    3. Extract 17 landmarks (ordering per ``landmark_order``)
    4. Interpolate short gaps (≤3 frames)
    4b. Temporal Gaussian smoothing (OFF by default, sigma=0)
    ...
    Args:
        ...
        smoothing_sigma: Gaussian std in frames for temporal smoothing of the
            ...
            :func:`smooth_landmark_series`.
        smoothing_filter: ``"gaussian"`` or ``"butterworth"`` (N20b).
        smoothing_cutoff_hz: Butterworth -3 dB cutoff; ``0.0`` disables it, so
            the default leaves behaviour unchanged regardless of
            ``smoothing_filter``.
        smoothing_order: Butterworth order (zero-phase via ``filtfilt``).
        fps: Frame rate, needed only for the Butterworth Hz conversion. All 45
            S1 clips are exactly 60.0.
        landmark_order: Which of ``LANDMARK_ORDERINGS`` to place the 17
            landmarks in. **Default ``"u_shape"`` reproduces
            every prior call bit-identically** — the current production
            body-contour ordering. ``"mediapipe_raw"`` and ``"fixed_random"``
            are the ablation's other two arms; the hip-centred normalisation
            anchor (left hip / right hip / left shoulder) is re-derived for
            whichever ordering is in effect via
            :func:`landmark_role_indices`, so scaling stays correct under
            any arm.
    ...
    # Step 3: Extract 17 landmarks (ordering per landmark_order) - raw pixels
    nn_landmarks, nn_visibilities = extract_nn_landmarks_raw(
        canon_lm, canon_vis, order=landmark_order
    )
    metadata["landmark_order"] = landmark_order
    ...
    # Step 4b: Temporal smoothing. No-op at the default sigma=0.
    nn_landmarks, smooth_meta = smooth_landmark_series(
        nn_landmarks,
        smoothing_sigma,
        filter_type=smoothing_filter,
        cutoff_hz=smoothing_cutoff_hz,
        order=smoothing_order,
        fps=fps,
    )
    metadata["smoothing"] = smooth_meta

    # Step 5: Hip-centered normalization. Role indices are re-derived from
    # landmark_order rather than relying on hip_centered_normalization's
    # u_shape-only defaults, so scaling stays correct under any ordering arm.
    l_hip, r_hip, l_shoulder = landmark_role_indices(landmark_order)

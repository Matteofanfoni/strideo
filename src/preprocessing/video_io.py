...
def load_video(
    ...
    # Calculate downsampling. Fast sources only: the max(1, ...) clamp passes a
    # SLOWER source through at its own rate instead of correcting it, which is
    # safe only because ensure_cfr normalises a constant-but-slow clip to
    # target_fps upstream.

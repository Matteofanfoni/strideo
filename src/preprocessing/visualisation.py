...
def draw_metrics_hud(
    img: np.ndarray, lines: List[str], font_scale: float = 1.0
) -> None:
    """Burn a semi-opaque metrics box (top-left) into ``img``, one line per
    string. Shared by the app's StrideoNet-overlay annotated video and
    ``scripts/render_strideonet_hero_overlay.py``'s hero-image HUD, so the
    look stays identical between the static asset and the on-demand video."""
    h, w = img.shape[:2]
    scale = max(0.6, min(1.4, w / 1920.0)) * font_scale
    line_h = int(38 * scale)
    box_w = int(560 * scale)
    box_h = line_h * len(lines) + int(22 * scale)
    overlay = img.copy()
    cv2.rectangle(overlay, (14, 14), (14 + box_w, 14 + box_h), (0, 0, 0), -1)
    cv2.addWeighted(overlay, 0.6, img, 0.4, 0, img)
    y = 14 + line_h
    for line in lines:
        put_text(
            img, line, (30, y), scale=0.78 * scale, thickness=max(1, int(2 * scale))
        )
        y += line_h

...
# BlazePose hip/ankle indices checked by the landmark-
# in-bounds contact gate. Matches the follow-up measurement script this gate
# was derived from exactly, so it
# reproduces its quantified numbers (223/586 contacts gated cohort-wide,
# cadence_std 23.70->13.39 spm, gct_std 33.93->13.13 ms) rather than a
# fresh, unverified reimplementation.
_BOUNDS_HIP_IDX = (LandmarkIndex.LEFT_HIP, LandmarkIndex.RIGHT_HIP)
_BOUNDS_ANKLE_IDX = (LandmarkIndex.LEFT_ANKLE, LandmarkIndex.RIGHT_ANKLE)


def gate_out_of_bounds_contacts(
    contacts: List[GroundContact],
    landmarks: np.ndarray,
    resolution_width: float,
    resolution_height: float,
) -> Tuple[List[GroundContact], Dict]:
    """Drop contacts whose hip/ankle landmarks sit outside the frame.

    The ``nan_to_num``/``np.interp``
    fabrication mechanism produces phantom contacts in
    the frames before the runner enters or after they exit shot — these
    survive every earlier pass (refine, prune, interpolate) because they
    still look like a plausible alternating-leg stride, and contaminate the
    raw persisted ``cadence_std``/``gct_std`` that the report-time
    zone gate does not protect. Runs last, after every contact-producing
    pass (including interpolation), so gated-out entry/exit contacts are
    not mistaken for holes and re-inserted by
    ``interpolate_missing_contacts``, and so the result matches what the
    original diagnosis measured against the final per-clip contact list.

    Checks BOTH hips (indices 23/24) AND BOTH ankles (27/28) at each
    contact's own frame, not just the leg that made contact — a strict
    superset of trimming to ``[first_detected, last_detected]`` (223/586
    vs 204/586 cohort-wide), since it also catches partial-body entry/exit
    strides a per-clip window can't see. A gated-out contact is dropped
    outright, not replaced with a coarse fallback — there is no real
    footstrike event in these frames to estimate a coarser value for; the
    landmarks there are fabricated because the runner was not in the shot
    at all, not because detection was merely imprecise.

    Args:
        contacts: contacts after every earlier pass.
        landmarks: (T, 33, 2) post-pipeline landmark tensor, pixel coords.
        resolution_width / resolution_height: frame bounds in pixels.

    **Frame selection is round-half-UP, not Python's ``round``**. A
    refined ``contact_frame`` is sub-frame, so this gate has to pick an integer
    frame to inspect -- and ``round`` implements banker's rounding, which
    resolves an exact ``.5`` by the *parity* of the neighbouring integer:
    ``round(40.5) == 40`` but ``round(41.5) == 42``. Whether the gate looks
    before or after a contact then depends on whether its coarse frame happens
    to be odd or even, which is not a property any decision should rest on.

    That was latent rather than harmless: no contact under ``v1.29`` landed on
    an exact ``.5``, so it never fired. The half-frame IC calibration makes
    the refinement fallback compute ``coarse - 0.5`` exactly, putting 13
    contacts across 11 clips precisely on the tiebreak -- and one of them
    (``S1_02_800m_3``, coarse frame 41) changed the clip's scored GCT by
    3.4 ms purely through which frame got inspected. ``floor(x + 0.5)`` is
    deterministic, ties resolving consistently to the later frame, and is
    identical to ``round`` for every non-``.5`` input, so it changes no stored
    result.

    Returns:
        (survivors, diagnostics). diagnostics: ``n_dropped``, ``drop_log``
        (list of per-dropped-contact dicts with frame / leg / which
        landmark(s) were out of bounds).
    """
    ...
    for c in contacts:
        contact_frame = c.contact_frame if c.contact_frame is not None else c.frame
        # Round half UP -- see the docstring: `round` would resolve an exact
        # .5 by parity of the neighbour, making the gate's verdict depend on
        # whether the coarse frame is odd or even.
        idx = min(max(math.floor(float(contact_frame) + 0.5), 0), n_frames - 1)
        out_of_bounds = []
        for side, hip_lm in zip(("L", "R"), _BOUNDS_HIP_IDX):
            x, y = landmarks[idx, hip_lm]
            if not (0 <= x <= resolution_width and 0 <= y <= resolution_height):
                out_of_bounds.append(f"{side}_hip")
        for side, ankle_lm in zip(("L", "R"), _BOUNDS_ANKLE_IDX):
            x, y = landmarks[idx, ankle_lm]
            if not (0 <= x <= resolution_width and 0 <= y <= resolution_height):
                out_of_bounds.append(f"{side}_ankle")
    ...
    diagnostics: Dict = {"n_dropped": len(drop_log), "drop_log": drop_log}
    return survivors, diagnostics


def contacts_in_presence_window(
    contacts: List[GroundContact],
    landmarks: np.ndarray,
) -> List[GroundContact]:
    """Restrict a coarse contact list to the runner's presence window.

    ``compute_presence_quality`` bounds the window by the frames where a hip
    and an ankle are actually detected, i.e. the runner's own transit. A
    contact outside it was built from frames with no runner in shot -- on
    ``S1_01_threshold_1``, 31 of the coarse pass's 38 contacts are of that
    kind, and every one of them classifies as 'forefoot', which is enough to
    outvote the 7 real, unanimously 'heel' contacts.

    Returns ``contacts`` unchanged when no frame has both a hip and an ankle,
    since there is then no window to filter against and dropping everything
    would be worse than voting on what is there.
    """
    # Function-local import: src.utils.quality imports src.utils.metrics,
    # which imports this module. A module-level import would be circular.
    from src.utils.quality import compute_presence_quality

    presence = compute_presence_quality(landmarks)
    w_lo, w_hi = presence["window_start"], presence["window_end"]
    if w_lo is None or w_hi is None:
        return contacts
    return [c for c in contacts if w_lo <= float(c.frame) <= w_hi]
...
def detect_clip_strike_pattern(
    ...
    rtm_landmarks: Optional[np.ndarray] = None,
    rtm_scores: Optional[np.ndarray] = None,
    use_refined_ic: bool = False,
    min_participation: float = 0.0,
) -> Dict:
    """Classify the dominant strike pattern for an entire clip — the
    fallback source used only when no strike is user-declared.

    One signal decides, and it is the only one computed: a majority vote of
    per-contact zero-crossing classifications across all detected contacts,
    taking the plurality.

    That vote was originally paired with two population priors -- pace
    (800m/1500m ⇒ forefoot) and footwear (track_spike ⇒ forefoot) -- which
    could 2-outvote it. They were severed from 'decision' once a caller
    could fall back to a user-declared strike, on the measurement
    that they run at 40-47%, i.e. *below chance* on a binary call, against
    75.0% for the vote where it commits. They were then left computed and
    returned for diagnostics, and **finally deleted outright.** A prior that
    never votes is not a diagnostic, and the shoe prior in particular has
    zero variance on our own cohort (every runner wears spikes), making it a
    constant rather than a weak signal. The concrete harm: that constant
    emitted 'forefoot' for an unambiguous heel striker.

    Deleting them changed no behaviour -- 'decision' had not consulted the
    priors for some time, so no strike classification, IC, metric or
    validation verdict moves. Only the returned schema shrinks:
    'speed_signal'/'shoe_signal' are gone. They survive, verbatim, in a
    retired-code module, which is what keeps their own published
    measurements reproducible.

    Returns a dict with the vote, its counts, the final 'decision'
    ('forefoot' | 'heel' | 'unknown') and a 'confidence' rating derived
    from the vote's own margin.
    """
    # --- Majority vote across contacts ---
    ...
    # --- Decide (the vote alone; the priors are deleted) ---
    # The vote alone decides. Confidence reflects the vote's own margin,
    # not cross-signal agreement.
    decision = vote_signal

    winning = vote_counts[decision] if decision != "unknown" else 0
    losing = classified - winning
    if decision == "unknown":
        confidence = "none"
    elif losing == 0:
        confidence = "strong"
    elif winning >= 2 * losing:
        confidence = "moderate"
    else:
        confidence = "weak"

    # The margin rule above ignores how many contacts voted at all.
    # `S1_05_800m_3` is the case that exposes it -- 1F/2H/6U, so two thirds
    # of contacts abstained and three decided the clip, yet `winning >= 2 *
    # losing` (2 >= 2) grades that **moderate** while grading a 3-vs-2 split
    # on five voters **weak**. It is also the only clip v1.26 misclassifies,
    # and the user confirmed it visually as a borderline/midfoot strike --
    # i.e. exactly the case where near-silence is the honest signal.
    #
    # `min_participation` demotes to 'weak' when the share of contacts that
    # actually cast a heel/forefoot vote falls below the given fraction.
    # 0.0 (the default) disables it and preserves the original grading.
    if min_participation > 0.0 and confidence in ("strong", "moderate"):
        if vote_total > 0 and (classified / vote_total) < min_participation:
            confidence = "weak"
...
def _detect_strike_pattern_for_contact(
    ...
    ``use_refined_ic`` reads the geometry at the *refined*
    touchdown (``contact.contact_frame``, rounded) instead of the coarse
    frame. The coarse frame is what this function has always used, and it is
    measurably the wrong place to look: across our cohort the coarse pass
    fires a mean **0.98 frames early**, and on one forefoot runner it fires
    ...
    """
    if use_refined_ic and contact.contact_frame is not None:
        F_IC = int(round(contact.contact_frame))
    else:
...
# Residual IC-late calibration.
#
# Measured on the 29 reference-matched, zone-gated, correctly-strike-classified
# contacts of our own cohort, *after* the break-point
# TO fix below (which does not touch IC): +8.1 ms late (heel +9.3, forefoot
# +6.5 -- no material strike dependence, sd 5.7 ms). Per that same diagnosis,
# this is not a detection defect
# to fix mechanically -- at 60 fps the landing landmark advances less than one
# RTMPose quantization step per frame at touchdown, so no sub-frame IC is
# recoverable from this signal at all; the crossing frame is decided by when
# the coordinate staircase happens to step, not by the foot.
#
# The correction is expressed in FRAMES, not milliseconds, because the
# mechanism above is a sampling-quantization one. A detect-on-first-observable-
# step process fires on average half a sample late by construction, which is a
# fixed fraction of a frame -- not a fixed time. The two coincide only at
# 60 fps: 8.1 ms is 0.486 frames there (a 0.233 ms difference from 0.5, far
# below the ~0.59 ms replication scale, so our cohort CANNOT distinguish them
# -- every clip in it is 60 fps). They diverge everywhere else: the old
# fixed-time form over-corrects ~2x at 120 fps and under-corrects to 0.243
# frames at 30 fps. This change is therefore justified by the mechanism, not
# by a measurable improvement on that cohort, and its payoff is correctness
# at any other
# frame rate. THE CLEAN FALSIFYING TEST is one clip shot at 120 fps: a
# residual near 4 ms confirms quantization and this parameterisation; near
# 8 ms refutes it and the fixed-time form was right.
#
...
#
# A single calibration constant is the correct response to a genuine sampling
# floor, legitimate here specifically because S1 (this pipeline's target
# population: multiple runners, race paces, track spikes) shows it uniform
...
_IC_LATE_BIAS_FRAMES = 0.5

# RETIRED, kept because two published measurements depend on it. Two analysis
# scripts import this and
# apply it as `td - _IC_LATE_BIAS_MS * fps / 1000.0` to reproduce the
# behaviour they were measured against. Repointing them at the frame form
# would silently change numbers those measurements already record, so the old
# constant stays exactly as it was -- the same precedent as the other
# retired-code modules kept for reproducibility.
# PRODUCTION NO LONGER READS THIS; the live path uses _IC_LATE_BIAS_FRAMES.
_IC_LATE_BIAS_MS = 8.1

# Forefoot IC / toe-off velocity thresholds (v1.22).
#
# These replace the v1.16 "hip-calibrated" thresholds, which multiplied a
# constant k (0.40 IC / 0.55 TO) by the clip-mean hip-x velocity on the
# stated rationale that this made them self-calibrating on pace. A later audit
# showed that velocity was computed over a flat-clamped, `np.interp`-filled
# hip trace and was therefore diluted by each clip's detection coverage --
# so the threshold tracked *coverage* (r=+0.851), not pace (r=+0.414), and
# swung roughly 3x across the cohort for no principled reason.
#
# Velocity and constants were then re-derived together, which is required
# because
# the two are entangled (correcting the velocity alone regresses GCT MAE
# 8.90 -> 23.87 ms). The result retires the pace term rather than re-fitting
# it: re-deriving k against a corrected velocity overfits badly
# (leave-one-clip-out 14.01 ms, worse than the defect it replaces) and fails
# to transfer to the second cohort's camera geometry -- the same failure mode
# that killed the first structured-offset attempt at the IC bias. A threshold
# with no velocity term at all was the only candidate to win on all three
# tests: in-sample 7.59 ms, leave-one-clip-out 8.35 ms (12/18 clips better),
# pre_validation 7.86 ms (2/2 clips better), against the shipped 8.90 / 8.90
# / 11.74. The rigid-leg tan(30-35 deg) justification is therefore retired,
# not repaired: it described a mechanism the shipped code never implemented.
#
# Expressed as a fraction of `resolution_height` with an fps term, matching
# the `velocity_threshold` / ankle-gate convention already used above, so
# they scale with capture format. The fitted values are px/frame at
# 2160p/60fps: IC 6.20, TO 7.60. Note the S1 and pre_validation cohorts are
...
def _fit_intersect(y: np.ndarray, ramp: range, plateau: float) -> Optional[float]:
    """Frame at which the least-squares line through ``y[ramp]`` reaches
    ``plateau``. ``None`` if fewer than 3 finite samples fall in ``ramp`` or
    the fit is degenerate (near-zero slope)."""
    idx = np.array(
        [i for i in ramp if 0 <= i < len(y) and np.isfinite(y[i])], dtype=float
    )
    if len(idx) < 3:
        return None
    ...
    if abs(slope) < 1e-6:
        return None
    return float((plateau - intercept) / slope)


def _refine_toe_off_breakpoint(
    toe_y: np.ndarray,
    ic_frame: float,
    coarse_to_frame: float,
    T: int,
    ramp_len: int = 3,
    gap: int = 1,
) -> Optional[float]:
    """Threshold-free toe-off refinement -- the frame at which the toe's
    post-liftoff free-flight ascent, extrapolated backwards as a straight
    line, reaches the stance plateau (this same landmark's own median height
    during stance). No threshold, no pixel scale, no pace/shoe constant.

    The diagnosis found both existing per-strike toe-off thresholds
    (``delta_lift_frac`` for heel, the forefoot liftoff ``k``) are correct in
    direction but wrong in *magnitude* in a way that diverges between S1 and
    pre_validation's camera geometries -- no single fixed value transfers.
    This construction instead measures the moment of departure directly from
    the landmark's own trajectory, using ``coarse_to_frame`` (the existing
    threshold-based estimate) only to place the ramp/plateau windows, not to
    determine the result -- a several-frame anchor error does not change the
    ramp fit or the plateau's own value.

    Returns ``None`` (caller keeps ``coarse_to_frame``) when the stance
    window is too short, or the ramp window has too few finite points to fit
    -- near a clip boundary, or under heavy occlusion.
    """
    ic0, to0 = int(round(ic_frame)), int(round(coarse_to_frame))
    if to0 - ic0 < 4:
        return None
    core = [i for i in range(ic0 + 2, to0 - 1) if 0 <= i < T]
    if len(core) < 2:
        return None
    plateau = float(np.nanmedian(toe_y[core]))
    ramp = range(to0 + gap, min(to0 + gap + ramp_len + 1, T))
    return _fit_intersect(toe_y, ramp, plateau)
...
def refine_contacts(
    ...
    - ``"forefoot"``, ``None``: toe (foot index) y-velocity falling
      zero-crossing in the symmetric window ``[C_IC−5, C_IC+5]``. Unchanged
      from v1.5–v1.14 — ``None`` means no strike-aware toe-off is in play
      at all (the byte-identical default paths), not "abstained".
    - ``"unknown"``: the clip-level vote abstained (mixed clip, tie,
      or no user-declared strike — only reachable when the caller is
      already running ``per_strike_pattern``). Resolves **per contact**
      via ``_detect_strike_pattern_for_contact`` instead of silently
      defaulting every contact to forefoot-style dispatch — mirrors the
      toe-off dispatch's own per-contact fallback below, so a clip with a
      genuine tie doesn't lose heel-style touchdown timing for the
      contacts that individually classify as heel.
    ...
    Args:
        ...
        clip_strike_pattern: Clip-level strike pattern ('forefoot' |
            'heel' | 'unknown' | None). Controls IC refinement signal and
            window (heel → heel y-velocity + forward-biased window;
            forefoot/None → toe y-velocity + symmetric window; 'unknown'
            → resolved per contact, then dispatched the same way).
            Also controls TO dispatch under ``per_strike_pattern``:
            'forefoot' → B-B2, 'heel' → B2c, 'unknown'/None → resolved
            per contact.
    ...
    for leg in ["L", "R"]:
        ...
        # Forefoot IC / toe-off thresholds (v1.22).  Fixed
        # px/frame values scaled to the capture format — no hip-velocity
        # term.  The v1.16 hip-calibrated form this replaces is documented
        # at _FOREFOOT_IC_THRESH_FRAC above, along with why the pace term
        # was retired rather than re-fitted against a corrected velocity.
        _thresh_scale = resolution_height * (60.0 / fps)
        velocity_threshold_forefoot_ic = _FOREFOOT_IC_THRESH_FRAC * _thresh_scale
        lift_threshold_forefoot = -_FOREFOOT_TO_THRESH_FRAC * _thresh_scale
        ...
        for contact in contacts:
            ...
            # unknown → resolved per contact via
            #              _detect_strike_pattern_for_contact, then
            #              dispatched exactly as heel/forefoot above.
            ...
            effective_strike_pattern: Optional[str]
            if clip_strike_pattern == "unknown":
                effective_strike_pattern = _detect_strike_pattern_for_contact(
                    contact,
                    landmarks,
                    visibilities,
                    (
                        body_height_px
                        if body_height_px is not None
                        else 0.4 * resolution_height
                    ),
                    tau_strike_frac=tau_strike_frac,
                ...
                effective_strike_pattern = clip_strike_pattern
            # "as_forefoot": collapse 'unknown' onto the forefoot path
            # rather than giving it one of its own. Forefoot is the robust
            # choice, not an arbitrary one: its primary TO method never
            # failed on this cohort (0/502 calls), while the heel primary
            # failed on 9 surviving contacts and every one of those was then
            # dropped by the sanity gate.
            if effective_strike_pattern == "unknown" and unknown_as_forefoot:
                effective_strike_pattern = "forefoot"

            def _ic_heel() -> Tuple[float, bool]:
                """Heel branch: backward search on heel y-velocity."""
                out = refined_touchdown
                found = False
                start = max(1, contact.frame - 2)
                end = min(T - 1, contact.frame + 10)
                for i in range(end, start, -1):
                    v_prev = heel_velocity[i - 1]
                    v_curr = heel_velocity[i]
                    if v_prev > velocity_threshold and v_curr <= velocity_threshold:
                        if v_prev != v_curr:
                            frac = (velocity_threshold - v_prev) / (v_curr - v_prev)
                            out = (i - 1) + frac
                        else:
                            out = float(i - 1)
                        found = True
                ...
                return max(out, float(contact.frame)), found

            def _ic_forefoot() -> Tuple[float, bool]:
                """Forefoot branch: forward search on toe y-velocity.

                First crossing = earliest toe arrest = true IC. No C_IC
                floor — forefoot C_IC fires AFTER true IC. Format-scaled
                threshold (v1.22): see velocity_threshold_forefoot_ic.
                """
                out = refined_touchdown
                found = False
                start = max(1, contact.frame - 5)
                end = min(T - 1, contact.frame + 5)
                for i in range(start + 1, end + 1):
                    ...
                    if (
                        ...
                        if v_prev != v_curr:
                            frac = (velocity_threshold_forefoot_ic - v_prev) / (
                                v_curr - v_prev
                            )
                            out = (i - 1) + frac
                        else:
            ...
            if effective_strike_pattern == "heel":
                refined_touchdown, _ = _ic_heel()
            elif effective_strike_pattern != "unknown":
                # 'forefoot', and also None -- the documented, protected
                # v1.5-v1.14 default, which must keep taking this branch
                # (and its test). Only a *genuine* 'unknown' falls
                # through to the least-displacement rule below.
                refined_touchdown, _ = _ic_forefoot()
            else:
                # User-specified: the strike is genuinely
                # undetermined for this contact, so take the branch that
                # displaces the coarse IC *least* -- the minimum-assumption
                # choice when there is no evidence to justify either.
                #
                # This replaces a silent default, not a safe one: 'unknown'
                # used to fall through to the else branch, i.e. it committed
                # to the forefoot/toe path without saying so. On a borderline
                # or midfoot strike (S1_05_800m_3, confirmed visually by the
                # user 2026-08-29) that is a coin flip presented as a
                # decision.
                #
                # Only branches that actually found a velocity crossing may
                # compete. Without that guard the heel branch would win by
                # construction: its C_IC floor returns exactly contact.frame
                # when no crossing is found, i.e. displacement 0.
                h_ic, h_found = _ic_heel()
                f_ic, f_found = _ic_forefoot()
                if h_found and f_found:
                    refined_touchdown = min(
                        (h_ic, f_ic), key=lambda v: abs(v - float(contact.frame))
                    )
                elif h_found:
                    refined_touchdown = h_ic
                elif f_found:
                    refined_touchdown = f_ic
            ...
            if toe_off_method == "ankle_y_lift":
                ...
                if clip_strike_pattern in ("forefoot", "heel"):
                    ...
                    if contact.strike_pattern == "unknown" and (unknown_as_forefoot):
                        contact.strike_pattern = "forefoot"
                ...
                if (
                    contact.strike_pattern in ("heel", "forefoot")
                ) and refined_toe_off is not None:
                    breakpoint_to = _refine_toe_off_breakpoint(
                        toe_y, refined_touchdown, refined_toe_off, T
                    )
                    if breakpoint_to is not None:
                        refined_toe_off = breakpoint_to

                # residual IC-late sampling-floor calibration
                # (see _IC_LATE_BIAS_FRAMES docstring above) -- applied only
                # now that TO's strike-dependent bias is fixed structurally
                # above, so this residual is genuinely uniform rather than a
                # threshold artifact entangled with it. Subtracted as a
                # fraction of a FRAME, so it needs no fps conversion and stays
                # correct at frame rates other than 60.
                if contact.strike_pattern in ("heel", "forefoot"):
                    refined_touchdown -= _IC_LATE_BIAS_FRAMES
...
def detect_ground_contacts(
    ...
    rtm_landmarks: Optional[np.ndarray] = None,
    rtm_scores: Optional[np.ndarray] = None,
    auto_clip_strike_pattern: bool = False,
    strike_from_refined_ic: bool = False,
    ...
    Args:
        ...
        auto_clip_strike_pattern: When True and
            ``clip_strike_pattern`` is None, classify the clip's strike
            pattern here, from *this* function's own coarse list, restricted
            to the runner's presence window. Callers used to do this
            themselves by calling ``detect_contacts_coarse`` a second time
            with different arguments; see the note below Pass 1 for why that
            was wrong. Off by default so no existing caller changes
            behaviour. The resulting dict is returned under the summary's
            ``clip_strike_pattern`` key.
        strike_from_refined_ic: After the coarse-frame
            vote, refine once with that provisional pattern, re-vote at the
            *refined* ICs, and adopt the second vote when it commits. Costs
            one extra refinement pass. Requires ``auto_clip_strike_pattern``
            and ``refine``; ignored otherwise. Off by default.
        demote_weak_strike_vote: Treat a ``weak`` vote
            margin as ``unknown`` rather than committing on it. ``unknown``
            is then resolved per contact by the least-coarse-IC-displacement
            rule in ``refine_contacts``. Requires
            ``auto_clip_strike_pattern``. Off by default.
        gate_landmark_bounds: Drop any contact whose hip/
            ankle landmarks are outside the frame at its own frame (entry/
            exit fabrication). Runs last, after every other pass. Requires
            ``resolution_width``.
    ...
    # Pass 1b: clip-level strike pattern, voted here.
    #
    # This used to live in the callers, which each called
    # detect_contacts_coarse a *second* time to build a list to vote over --
    # and did so without rtm_landmarks/rtm_scores or the horizontal-velocity
    # gate, so the vote was cast over a different detector's output than the
    # one being refined below. Worse, it voted over the whole clip: on
    # S1_01_threshold_1 the coarse pass returns 38 contacts, only 7 of which
    # fall inside the runner's presence window, and all 31 outside it are
    # labelled 'forefoot' -- fabricated from frames with no runner in shot.
    ...
    # filter applies to the vote only -- the contacts handed to refinement,
    # pruning and the bounds gate below are untouched, since narrowing
    # those is a separate question.
    clip_strike_signals: Optional[Dict] = None
    if auto_clip_strike_pattern and clip_strike_pattern is None:
        voting_contacts = contacts_in_presence_window(contacts, landmarks)
        clip_strike_signals = detect_clip_strike_pattern(
            voting_contacts,
            landmarks,
            visibilities,
            body_height_px if body_height_px else 0.4 * resolution_height,
            tau_strike_frac=tau_strike_frac,
            rtm_landmarks=rtm_landmarks,
            rtm_scores=rtm_scores,
            min_participation=min_participation,
        )
        clip_strike_signals["n_voting_contacts"] = len(voting_contacts)
        clip_strike_signals["n_coarse_contacts"] = len(contacts)
        clip_strike_pattern = clip_strike_signals["decision"]

        # Pass 1c: re-vote at the *refined* IC.
        #
        # Windowing the vote (above) removes a genuine defect -- 498 of 499
        # out-of-window contacts across the cohort vote 'forefoot' regardless
        # of the runner, a constant with zero variance of exactly the kind
        # the retired shoe prior was. But removing it exposed a second,
        # independent defect it had been masking: the coarse pass fires a
        # mean 0.98 frames early cohort-wide (2-4 frames on the forefoot
        # runner S1_05), and the classifier reads foot geometry at the coarse
        # frame, i.e. while the foot is still in swing. Windowed coarse
        # frames score 34/45 clips against Kinovea; contacts placed at the GT
        # IC score 43/45. The logic is sound, the input frames were not.
        #
        # So: refine once with the provisional pattern, re-vote at the
        # refined ICs, and if that vote *commits* to something different,
        # refine again from the untouched coarse list. An 'unknown' second
        # vote is not allowed to override a committed first one -- at the
        # true IC the heel and toe are near level and 42% of contacts read
        # 'unknown', so silence there is absence of evidence, not evidence.
        # The trial refinement is discarded; only its vote is kept.
        if refine and strike_from_refined_ic:
            trial = copy.deepcopy(contacts)
            trial, _ = refine_contacts(
                trial,
                landmarks,
                visibilities,
                fps,
            ...
            second = detect_clip_strike_pattern(
                contacts_in_presence_window(trial, landmarks),
                landmarks,
                visibilities,
                body_height_px if body_height_px else 0.4 * resolution_height,
                tau_strike_frac=tau_strike_frac,
            ...
            clip_strike_signals["provisional"] = {
                "decision": clip_strike_pattern,
                "vote_counts": dict(clip_strike_signals["vote_counts"]),
                "confidence": clip_strike_signals["confidence"],
            }
            clip_strike_signals["refined_ic_vote"] = second
            if second["decision"] != "unknown":
                clip_strike_pattern = second["decision"]
                clip_strike_signals["decision"] = second["decision"]
                clip_strike_signals["confidence"] = second["confidence"]
                clip_strike_signals["vote_signal"] = second["vote_signal"]
                clip_strike_signals["vote_counts"] = second["vote_counts"]
            clip_strike_signals["changed_by_refined_ic"] = (
                clip_strike_signals["provisional"]["decision"] != clip_strike_pattern
            )

        # A 'weak' margin is not a decision.
        #
        # 'confidence' had long been computed and returned, and consumed
        # by nothing -- `decision = vote_signal` regardless of whether the
        # margin was 9-0 or 5-4. The classifier measures 75% *where
        # it commits*, and a one-vote margin is barely committing. Demoting
        # weak to 'unknown' hands the contact to the least-displacement rule
        # in refine_contacts rather than to a coin flip.
        if demote_weak_strike_vote and clip_strike_signals is not None:
            if clip_strike_signals["confidence"] == "weak":
                clip_strike_signals["demoted_from"] = clip_strike_pattern
                clip_strike_pattern = "unknown"
                clip_strike_signals["decision"] = "unknown"
    ...
    if clip_strike_signals is not None:
        summary["clip_strike_pattern"] = clip_strike_signals
    ...
    # Pass 5: drop contacts whose hip/ankle landmarks are
    # out of frame at their own frame (entry/exit fabrication). Runs last,
    # after interpolation, so a gated-out entry/exit contact isn't mistaken
    # for a hole and re-inserted by Pass 4.
    if gate_landmark_bounds:
        if resolution_width is None:
            raise ValueError(
                "--gate-landmark-bounds requires resolution_width (pass via "
                "detect_ground_contacts)."
            )
        contacts, gate_diag = gate_out_of_bounds_contacts(
            contacts,
            landmarks,
        ...
        summary["total_contacts"] = len(contacts)
        summary["gate_landmark_bounds"] = gate_diag

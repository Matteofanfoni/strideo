...
# warning, so this note is the guard.
#
# The reason is the sign defect documented in create_spatial_calibration below.
# pixels_per_cm reads A/(H - s) where it should read A/H, so every spatial
# metric reads low by exactly s/(H - s) -- strictly proportional to the constant
# used here. Measured at H=175cm:
#
#     barefoot 0.0cm -> 0.000%    training_shoe 2.5cm -> 1.449%
#     track_spike 1.0cm -> 0.575%   super_shoe 3.5cm -> 2.041%
#     racing_flat 1.5cm -> 0.865%
#
# So while the defect stands, these understated values are *accidentally
# cancelling part of it*, and raising them removes that protection (raising
# track_spike to 1.5 alone would take S1 from 0.575% to ~0.86%). They are kept
# low deliberately, not by oversight.
#
# Revisit only together with the shoe-term defect: once it leaves the denominator its
# value stops affecting pixels_per_cm at all, and these can then be corrected to
# their real ranges purely on their own merits (they still feed body_height_cm
# for the v1.4 anatomy gate, and final_foot_px -> leg_length, where accuracy
# genuinely helps).
SHOE_TYPES = {
...
def create_spatial_calibration(
    ...
    # not reach pixels_per_cm at all. Cf. the correctly-signed sibling
    # ``body_height_cm = runner_height_cm + shoe_sole_cm`` in pipeline.py.
    # Measured effect: pixels_per_cm reads ~0.575% high, so every spatial metric
    # reads ~0.575% low. Left in place because no available measurement can
    ...
    # and the two must move together -- see the note there before editing
    # either one.
    ...
    #
    # Second half of the defect above, and the reason the two
    # compound instead of cancelling: total_height_px now includes the sole, so
    # it is a *shod* height in pixels, but it is divided by runner_height_cm, a
    # *barefoot* height in cm. Mismatched on one line. Correctly anchored this
    # would read ``/ (runner_height_cm + shoe_sole_cm)``, and then
    # ``A/H == (A + s*ppc)/(H + s)`` holds exactly.
    #
    # As written the two errors collapse to ppc = A / (H - s), which makes this
    # whole recalculation an exact algebraic no-op -- it returns precisely what
    # the division above already produced (verified on 45/45 clips). It is
    # kept because final_segments/final_foot_px are themselves used downstream
    # (leg_length_px -> leg_length_m, the normalised ratios), not for its effect
    # on pixels_per_cm, which is nil.
    #
    # WHEN FIXING -- THE TWO HALVES MUST MOVE TOGETHER.
    # Correcting only one site replaces one mismatch with another and breaks
    # the no-op property above, which is the thing that currently makes this
    # recalculation harmless. Either:
    #   (a) anatomical_height_cm = runner_height_cm   (barefoot / barefoot)
    #       AND this line -> total_height_px / (runner_height_cm + shoe_sole_cm)
    #       (shod / shod), or
    #   (b) drop this recalculation entirely and keep ppc = A / H, retaining
    #       final_segments purely for its downstream consumers.
    # Both give the same pixels_per_cm, since A/H == (A + s*ppc)/(H + s) holds
    # exactly. Verify after any change that this recalculation is still a no-op
    # (it was on 45/45 clips) -- if it starts moving pixels_per_cm, the two
    # halves have been left inconsistent.
    #
    # Note what the fix does NOT remove: shoe_sole_cm keeps two live,
    # correctly-signed roles afterwards. (1) shoe_sole_px above -> final_foot_px
    # -> leg_length_px, which reaches the spurious-contact pruning gate
    # (pace_estimator.step_length_pred = leg_length_m * (1.4 + 0.1*v)), the
    # oscillation/stride leg ratios, quality.py's stride_leg_ratio band, and the
    # app's oscillation display. (2) ``body_height_cm = runner_height_cm +
    # shoe_sole_cm`` in pipeline.py, which scales the bidirectional combine and
    # the strike/toe-off thresholds. So this is a scale-anchoring fix only, not
    # a removal of footwear from the pipeline.

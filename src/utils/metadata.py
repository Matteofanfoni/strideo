...
Reference: the data-collection protocol, metadata sections.
"""
...
class PersonalBests(BaseModel):
    ...
    pb_3000m: Optional[str] = Field(None, alias="3000m")
    pb_5k: Optional[str] = Field(None, alias="5k")
    pb_10k: Optional[str] = Field(None, alias="10k")
...
class EfficiencyMetrics(BaseModel):
    ...
    # Renamed from running_economy_index 2026-08-31, and redefined in the same
    # change (denominator moved from a full stride to a step, so values
    # doubled). The key changes with the meaning on purpose: a result JSON
    # written before that date carries the old name AND the old definition, so
    # a reader can tell them apart instead of silently mixing two scales under
    # one key. Legacy readers convert with x2 (same numerator, half the
    # denominator, so the conversion is exact).
...
class QualityOutput(BaseModel):
    """Quality/reliability indicators for the pipeline run.

    ``reliability_score``/``is_reliable`` replace the old ``overall_score``
    composite — see
    ``src.utils.quality.assess_clip_reliability``. ``metric_warnings``
    stays purely diagnostic text; it carries no weight in the score.

    ``presence_quality_ok``/``excluded`` are a second, independent gate
    (it replaced an earlier ``zone_visibility_ok``, whose entry/mid/exit
    mechanism was retired): ``None``/``False`` when
    presence data wasn't available to compute it, else ``False``/``True``
    when the runner's presence window is undefined or carries an interior
    detection dropout longer than gap-fill will repair — a failure mode
    ``is_reliable`` alone was found to miss. ``excluded`` is the
    authoritative "this clip's output should not be trusted" signal.
    """
